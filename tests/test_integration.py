...
def test_connect_juniper_switch(ip):
    connected = juniperconnect(ip)
    return connected

def test_connect_cisco_switch(ip):
    connected = ciscoConnect(ip)
    return connected

def test_config_SNMP_juniper_switch(ip, configuration):
    connected = juniperConfig(ip, configuration)
    return connected

def test_config_SNMP_Cisci_switch(ip, configuration):
    connected = ciscoConfig(ip, configuration)
    return connected

test_get_host(host_name='Zabbix server')
