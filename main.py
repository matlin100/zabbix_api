...
def enable_SNMP_to_juniper_switched(juniper_ip_list):
    for switch in juniper_ip_list:
        if not config_SNMP_juniper_switch(ip=switch['ip'], configuration=enable_juniper_snmp_config(switch['ip'])):
            return False
    return True

def enable_SNMP_to_cisco_switched(cisco_ip_list):
    for switch in cisco_ip_list:
        if not config_SNMP_Cisci_switch(ip=switch['ip'], configuration=enable_cisco_snmp_config):
            return False
    return True
    return True
...
def create_zabbixHost_to_all_switched(ip_list , swithc_company_name, template_id):
    for switch in ip_list:
        ...
        detals = create_host(host_name=f'{swithc_company_name}ip{last_octate}', ip=ip, template_id=template_id)
        print(detals)
